...
describe("Deposit", () => {
  it("Should make single deposit", async () => {
    ...
    expect(await sale.dexAmount()).to.equal(2000);
  });
  ...
  it("Should make multiple deposits", async () => {
    ...
    expect(await sale.dexAmount()).to.equal(21000);
    await sale.deposit(63000);
    expect(await sale.saleAmount()).to.equal(84000);
    expect(await sale.dexAmount()).to.equal(42000);
  });
...
describe("Sales", () => {
  ...
  it("Should perform multiple sales", async () => {
    ...
    expect(await sale.balanceOf(buyer1)).to.equal(ethers.parseEther("2000000"));
    expect(await sale.balanceOf(buyer2)).to.equal(ethers.parseEther("1000000"));
...
describe("Withdraw", () => {
  it("Should revert if locked", async () => {
    ...
    await expect(sale.connect(buyer1).withdraw()).to.be.revertedWithCustomError(
      sale,
      "ETokensAreLocked"
    );
  });

  it("Should not withdraw if nothing", async () => {
    const { sifa, buyer1, sale, start, end, unlock } = await loadFixture(
      deployPublicSale
    );
    await sifa.approve(sale, ethers.parseEther("15000000"));
  ...
  it("Should withdraw correct amount", async () => {
    ...
    expect(await sifa.balanceOf(buyer1)).to.equal(ethers.parseEther("600000"));
  });
});
